...
export const Game = {
  ...
  init(){
    ...
    document.querySelectorAll('.tab').forEach(el=>{
      el.onclick=()=>{ State.tab=el.dataset.tab; document.querySelectorAll('.tab').forEach(t=>t.classList.remove('active')); el.classList.add('active'); UI.render(); };
    });
    document.getElementById('vuln').onclick=()=> Events.hit();
  },
  bind(){
    ...
    document.getElementById('ocBtn').onclick=()=> this.overclock();
  },
  hack(){
    const now=Date.now();
    if(now-State.lastClick < 1500){ State.combo++; if(State.combo>10) State.combo=10; }
    else State.combo=0;
    State.lastClick=now;
  ...
  buyUp(i){
    ...
    if(!u.bought && State.cash>=u.cost){
      State.cash-=u.cost; u.bought=true; UI.log('upgrade '+u.name); UI.update();
    }
  },
  claim(i){
  ...
  overclock(){
    ...
    UI.log('OVERCLOCK 5x 10s');
    setTimeout(()=>{ State.overclockActive=false; State.overclockCd=60; },10000);
  },
  loop(){
    setInterval(()=>{
      const cps=getCPS();
